"""
foa_v1_niching.py - FOA avec Niching (fitness sharing)
"""
...
def compute_metrics(alpha, O, elapsed_time):
    ...
    # Calcul basé sur l'occupation des canaux
    channel_occupancy = np.sum(alpha, axis=0)  # nombre de SUs par canal
    busy_channels = np.sum(channel_occupancy > 1)  # canaux avec collisions
    total_channels = M
...
def plot_convergence(history, save_path='convergence_v1.png'):
    ...
    try:
        ...
        plt.ylabel('Fitness')
        plt.title('Convergence de FOA V1 (Niching)')
...
def run_foa_v1(verbose=True):
    """Exécute FOA avec Niching (V1)."""
    ...
    if verbose:
        print("=" * 60)
        print("FOA V1 - Allocation spectrale CRN (avec Niching)")
        print("=" * 60)
        print(f"Population: {POPULATION}, Itérations: {ITERATIONS}")
        print(f"Sigma_share: {SIGMA_SHARE}, Alpha_share: {ALPHA_SHARE}")
    ...
    if verbose:
        ...
        print(f"Meilleure fitness partagée: {shared_fitnesses[best_idx]:.4f}")
    ...
    for iteration in range(ITERATIONS):
        ...
        # Évaluation brute
        new_raw, new_alloc = evaluate_population(new_population, O, repair=True)
        
        # Niching
        new_shared = compute_shared_fitness(new_population, new_raw)
        
        # Meilleure solution (basée sur fitness partagée)
        best_shared_idx = np.argmax(new_shared)
        if new_raw[best_shared_idx] > best_fitness:
            best_fitness = new_raw[best_shared_idx]
            best_position = new_population[best_shared_idx].copy()
            best_allocation = new_alloc[best_shared_idx].copy()
            if verbose:
                print(f"✨ Itération {iteration}: nouvelle meilleure fitness brute = {best_fitness:.4f}")
